...
async function loadLessons(idx) {
  ...
  try {
    ...
    c.loadError = false;
    const completed = progressCache.get(progressDocId(c.id));
    if (completed) {
      const validLessonIds = new Set(c.videos.map((video) => video.id));
      progressCache.set(
        progressDocId(c.id),
        new Set([...completed].filter((id) => validLessonIds.has(id)))
      );
    }
  } catch {
    c.videos = [];
...
function getCourseCompleted(course) {
  return progressCache.get(progressDocId(course?.id)) || new Set();
}

async function completeLesson(course, lessonId) {
  if (!course || !isSafeDocId(lessonId)) return;
  const completed = getCourseCompleted(course);
  if (completed.has(lessonId)) return;
  completed.add(lessonId);
  progressCache.set(progressDocId(course.id), completed);
  if (caState.courseIdx !== null && COURSES[caState.courseIdx]?.id === course.id) {
        paintProgressSummary(course, completed);
        const button = document.querySelector(`[data-complete="${CSS.escape(lessonId)}"]`);
        if (button) {
          button.textContent = "✓";
          button.closest(".lesson-row")?.classList.add("is-complete");
        }
  }
  try {
    await saveCourseProgress(course, completed);
  } catch {
...
function openLesson(ci, vi) {
  ...
  document
    .querySelector("#caVideoBox video")
    ?.addEventListener("ended", () => completeLesson(c, v.id));
}

// المشغلات المضمّنة قد ترسل حدث النهاية إلى الصفحة عبر postMessage.
window.addEventListener("message", (event) => {
  const iframe = document.querySelector("#caVideoBox iframe");
